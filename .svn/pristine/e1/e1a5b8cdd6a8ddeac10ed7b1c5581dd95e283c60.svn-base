    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            let alertAction = UIAlertAction(title: "Ok", style: .destructive, handler: {[weak self] (action) in
                ...
                strongSelf.fireBaseRef.child("Users").child((strongSelf.userCurrent?.id)!).child("listFriend").child(userDel.id).removeValue()
                //Xoá bạn ở listFriend user bị xoá
                strongSelf.fireBaseRef.child("Users").child(userDel.id).child("listFriend").child((strongSelf.userCurrent?.id)!).removeValue()
                //strongSelf.dataList.remove(at: indexPath.row)
            })
            let cancelAction = UIAlertAction(title: "Cancel",
                                             style: .default) { (action: UIAlertAction) -> Void in
            }
            self.showAlert(title: "Xoá Bạn", msg: "Bạn có chắc muốn xoá bạn?", actions: [alertAction,cancelAction])
